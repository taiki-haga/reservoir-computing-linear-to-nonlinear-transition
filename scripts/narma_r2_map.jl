###############################################################
# NARMA task with an Echo State Network (ESN)
...
# Generate the target sequence for the NARMA task
function compute_narma_target(u_data::Vector{Float64}, m::Int, p::Vector{Float64})
    n = size(u_data, 1)
    d_data = zeros(n)
    for i ∈ m:n-1
        sigma = 0.0
        ...
        d_data[i+1] = p[1] * d_data[i] + p[2] * d_data[i] * sigma + p[3] * u_data[i-m+1] * u_data[i] + p[4]
    end
    return d_data
end
...
# Simulation of the NARMA task with an ESN
function run_narma_simulation(recurrent_matrix::SparseMatrixCSC, num_nodes::Int, input_std::Float64, noise_std::Float64, seed::Int)
    # Simulation steps
    ...
    train_targets_full = compute_narma_target(Float64.(u_train), m, p)
    ...
    test_targets_full = compute_narma_target(Float64.(u_test), m, p)
...
spectral_radius_grid = 0.1:0.025:0.95    # grid of spectral radii for the recurrent matrix
...
for seed in 1:10
    ...
    @showprogress for (i, spec_radius) in enumerate(spectral_radius_grid)
        for (j, input_std) in enumerate(input_std_grid)
            ...
            r2 = run_narma_simulation(rec_matrix, num_reservoir_nodes, input_std, noise_std, seed)
            r2_results[i, j] = r2
        end
    end
