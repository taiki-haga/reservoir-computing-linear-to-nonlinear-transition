...
for seed in 1:10
    ...
    @showprogress for (i, input_std) in enumerate(input_std_grid)
        recurrent_matrix = generate_recurrent_matrix(num_nodes, connectivity, spec_radius, seed)
        r2 = run_lorenz_simulation(recurrent_matrix, num_nodes, input_std, noise_std, seed)
        r2_results[i] = r2
    end
    # --- Write results for this seed ---
    write_to_file("r2_N=$(num_nodes)_seed=$(seed).txt", r2_results)
end
